...
class HtmlValidationTest extends TestCase
{
    ...
    public static function htmlFileProvider(): array
    {
        ...
        $files = self::findHtmlFiles($distDir);
        ...
        foreach ($files as $file) {
            // Use relative path from dist/ as key for readable test output
            $key = substr($file, strlen($distDir) + 1);
            $result[$key] = [$file];
        }
    ...
    public function testHtmlFilesWereGenerated(): void
    {
        ...
        $files = self::findHtmlFiles(self::DIST_DIR);
    ...
    public function testInternalLinksResolve(string $file): void
    {
        ...
        foreach ($matches[1] as $href) {
            ...
            // For root-relative links, resolve against dist/
            $cleanHref = ltrim($href, '/');

            // Directory paths should resolve to index.html
            if (str_ends_with($cleanHref, '/')) {
                $cleanHref .= 'index.html';
            }

            $target = $distDir . '/' . $cleanHref;
     ...
     * @return string[]
     */
    private static function findHtmlFiles(string $dir): array
    {
        $files = [];
